...
<div class="search-container">
    ...
    if not usersession then response:forward"//ProjectApplication/login.lsp" end
        function checkLogin()
            if not usersession.loggedin then
                print "not logged in"
                request:session(false)
                response:forward"//ProjectApplication/login.lsp"
            end
        end
checkLogin()
                    ...
                    <table class="table-users table" border="0">
                        ...
                        if (tonumber(usersession.viewAllUsers) == 1) then
                            ...
                            if request:method() == "POST" and request:data().companyName ~= "" then
                                sql = selectQueryWhere({"username","companyname","permissionlevel"},"users","companyname", request:data().companyName)
                            end
                        else 
                        ...
                        local function exec(cur)
                            ...
                            while Email do
                                ?>
                            <tr class='clickable-row' data-href='usersettings.lsp?Email=<?lsp=Email?>'>
