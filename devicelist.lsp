...
$(function() {
    ...
    function devInfo(info, ptid) {
        ...
        $("#deviceInfoTab-"+ptid).click(function(ev) {
            ...
            $("#deviceSettingsTab-"+ptid).removeClass("active").addClass("");
        ...
        $.ajax({
            ...
            success: function(output) {
                ...
                //$("#deviceSettings-"+ptid).load("graphload.lsp");
            ...
            $("#deviceSettingsTab-"+ptid).removeClass("").addClass("active");
            $("#deviceLogsTab-"+ptid).removeClass("active").addClass("");
        });
       ...
       devicetabhtml += '</table></div>';
        ...
        devicetabhtml +='<button class="submit-btn" type = "submit" id = "removeDevice'+ptid+'">Remove Device</button></form>';
        ...
        $("#removeDevice"+ptid).click(function(ev) {
            $.ajax({
              ...
              data: {deviceIP: $("#companyInput"+ptid).val(),deviceIP:info.ipaddr},
            success: function(output) {
                  alert(output);
                  if(--connectedDevs == 0)
                printNoDevs();
        ...
        smq.observe(ptid, function() {
            $('#dev-'+ptid).remove();
            $('#device-'+ptid).remove();
            $('#deviceContents-'+ptid).html('<h3>The device has disconnected<button id ="backtabdelete-'+ptid+'">Go back</button></h3>');
